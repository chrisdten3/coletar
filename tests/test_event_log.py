...
async def test_create_writes_exactly_one_row():
    store = InMemoryStore()
    await store.put_object(Memory.from_write("A durable fact."))
...
async def test_update_writes_exactly_one_row():
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("Chris uses pip."))
...
async def test_a_read_object_cannot_mutate_the_graph_behind_the_log():
    ...
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("The stored wording."))

    fetched = await store.get_object(obj.id)
    assert fetched is not None
    fetched.content = "Mutated without an event."

    unchanged = await store.get_object(obj.id)
    assert unchanged is not None and unchanged.content == "The stored wording."
    assert len(await store.list_events(object_id=obj.id)) == 1


async def test_retire_writes_exactly_one_row():
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("Superseded soon."))
    await store.retire_object(obj.id, reason="superseded")

    events = await store.list_events(object_id=obj.id)
...
async def test_edge_creation_writes_exactly_one_row():
    store = InMemoryStore()
    a = await store.put_object(Memory.from_write("One."))
    b = await store.put_object(Memory.from_write("Two."))
...
async def test_write_event_captures_before_and_after_state():
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("Chris works at Acme."))
    obj.content = "Chris works at Globex."
    await store.put_object(obj)

    created, updated = (await store.list_events(object_id=obj.id))[::-1]
...
async def test_a_caller_supplied_event_still_gets_before_and_after():
    ...
    event = (await store.list_events(object_id=memory.id))[0]
...
async def test_reading_the_log_hands_out_copies():
    ...
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("The original wording."))

    handed_out = (await store.list_events(object_id=obj.id))[0]
    assert handed_out.after is not None
    handed_out.after["content"] = "Tampered."

    again = (await store.list_events(object_id=obj.id))[0]
...
async def test_replay_reproduces_state_as_of_a_past_timestamp():
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("Chris works at Acme.", kind=MemoryKind.FACT))
    ...
    v2 = (await store.put_object(obj)).model_dump(mode="json")
    await asyncio.sleep(0.01)
    ...
    assert (await replay_object(store, obj.id, at=after_v1)).model_dump(mode="json") == v1
    assert (await replay_object(store, obj.id, at=after_v2)).model_dump(mode="json") == v2

    now_state = await replay_object(store, obj.id)
    assert now_state is not None and not now_state.is_active
    # Replay reads only the log, so it must agree with the object table.
    live = await store.get_object(obj.id)
...
async def test_replay_before_an_object_existed_returns_nothing():
    ...
    await asyncio.sleep(0.01)
    obj = await store.put_object(Memory.from_write("Later."))

    assert await replay_object(store, obj.id, at=before_anything) is None


async def test_replay_history_is_the_inspector_timeline():
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("Draft one."))
    obj.content = "Draft two."
    await store.put_object(obj)
    await store.retire_object(obj.id, reason="compressed")

    history = await replay_history(store, obj.id)
...
async def test_access_events_are_not_revisions():
    ...
    store = InMemoryStore()
    obj = await store.put_object(Memory.from_write("Read me."))
    await store.append_event(
        Event(type=EventType.OBJECT_ACCESSED, object_id=obj.id, actor=Actor.MODEL)
    )

    assert len(await replay_history(store, obj.id)) == 1
