...
async def test_prompt_block_carries_confidence_and_origin():
    store = InMemoryStore()
    await store.put_object(Memory.from_write("Chris ships C++ for EventBook."))
    result = await retrieve(store, "what does chris ship")
...
async def test_empty_result_injects_nothing():
    result = await retrieve(InMemoryStore(), "anything")
...
async def test_token_budget_truncates():
    store = InMemoryStore()
    for i in range(20):
        await store.put_object(Memory.from_write(f"Chris likes topic number {i} " + "x" * 200))
    result = await retrieve(store, "chris likes topic", top_k=20, token_budget=120)
